...
describe('Progress', () => {
  describe('线性进度条', () => {
    ...
    it('应该支持自定义格式化', () => {
      render(() => (
        <Progress percent={50} format={(p) => `已完成 ${p}%`} />
      ))
  ...
  describe('尺寸', () => {
    it('应该支持 sm 尺寸', () => {
      const { container } = render(() => <Progress percent={50} size="sm" />)
      const track = container.querySelector('.h-1')
      expect(track).toBeInTheDocument()
    })

    it('应该支持 lg 尺寸', () => {
      const { container } = render(() => <Progress percent={50} size="lg" />)
  ...
  describe('分段进度条', () => {
    it('应该渲染分段进度条', () => {
      const { container } = render(() => <Progress.Steps steps={5} current={3} />)
      const steps = container.querySelectorAll('.rounded-full')
      expect(steps).toHaveLength(5)
    })

    it('已完成的步骤应该有正确的颜色', () => {
      const { container } = render(() => <Progress.Steps steps={5} current={2} />)
