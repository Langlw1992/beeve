...
export const SwitchPage: Component = () => {
  return (
    <div class="space-y-12">
      <div>
        <h1 class="text-3xl font-bold">Switch</h1>
        <p class="text-muted-foreground mt-2">开关组件，用于切换某个功能的开启/关闭状态。</p>
      </div>
      ...
      <ShowcaseGrid
        ...
        renderCell={(size, state) => (
          <Switch size={size} checked={state === 'on'}>
            {state}
          </Switch>
        )}
      />
      ...
      {/* With Labels */}
      <ShowcaseSection title="With Labels" description="带标签的开关">
        <div class="flex flex-col gap-4">
          <For each={sizes}>
            {(s) => (
              <Switch size={s.value} defaultChecked>
      ...
      {/* Without Labels */}
      <ShowcaseSection title="Without Labels" description="不带标签的开关">
        <div class="flex items-center gap-4">
          <For each={sizes}>
            {(s) => <Switch size={s.value} defaultChecked />}
          </For>
        </div>
      </ShowcaseSection>
