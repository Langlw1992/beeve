...
export const SliderPage: Component = () => {
  return (
    <div class="space-y-12">
      ...
      {/* Single Value */}
      <ShowcaseSection title="Single Value" description="单值滑块">
        <div class="flex flex-col gap-6 max-w-md">
          <For each={sizes}>
            {(s) => (
              <div class="flex flex-col gap-2">
                <label class="text-xs text-muted-foreground">{s.label}</label>
                <Slider size={s.value} defaultValue={[30]} />
              </div>
            )}
          </For>
        </div>
      ...
      {/* Range Mode */}
      <ShowcaseSection title="Range Mode" description="范围滑块">
        <div class="flex flex-col gap-6 max-w-md">
          <For each={sizes}>
            {(s) => (
              <div class="flex flex-col gap-2">
                <label class="text-xs text-muted-foreground">{s.label}</label>
                <Slider size={s.value} defaultValue={[20, 70]} />
              </div>
            )}
          </For>
        </div>
      ...
      {/* With Marks */}
      <ShowcaseSection title="With Marks" description="带刻度标记">
      ...
      {/* With Input */}
      <ShowcaseSection title="With Input" description="带输入框">
        <div class="flex flex-col gap-6 max-w-lg">
          <Slider defaultValue={[50]} showInput />
          <Slider defaultValue={[20, 80]} showInput inputWidth="50px" />
        </div>
      </ShowcaseSection>

      {/* With Label */}
      <ShowcaseSection title="With Label" description="带标签">
        <div class="flex flex-col gap-6 max-w-md">
          <Slider label="Volume" defaultValue={[75]} />
          <Slider label="Price Range" defaultValue={[100, 500]} min={0} max={1000} showInput />
        </div>
      </ShowcaseSection>
